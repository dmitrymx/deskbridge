...
[![Version](https://img.shields.io/badge/version-2.7.0-blue?style=for-the-badge)](https://github.com/dmitrymx/deskbridge/releases)
...
| Возможность | v1.0.0 | v2.2.0 | v2.6.1 | v2.7.0 (Текущая) |
| :--- | :---: | :---: | :---: | :---: |
...
| **🆕 Общий буфер обмена (текст)** | ❌ | ❌ | ✅ | ✅ (UI-поток) |
| **🆕 Общий буфер обмена (файлы)** | ❌ | ❌ | ✅ (до 100MB) | ✅ (P2P-стриминг) |
| **🆕 Передача папок и файлов 3GB+** | ❌ | ❌ | ❌ | ✅ (без лимитов) |
| **🆕 Настройка скорости курсора** | ❌ | ❌ | ✅ | ✅ |
| **🆕 Настройка скорости прокрутки** | ❌ | ❌ | ✅ | ✅ |

---

## 🛠 Что нового в версии v2.7.0

*   **🚀 Выделенный P2P Clipboard-канал (порт 53204):** Передача файлов буфера обмена вынесена на отдельный порт. Управление мышью и клавиатурой не замерзает и работает идеально плавно даже во время скачивания больших файлов.
*   **📦 Потоковый P2P-стриминг чанками по 64 КБ:** Файлы транслируются с диска кусками по 64 КБ, не загружаясь целиком в RAM. Потребление памяти снижено с гигабайт до стабильных ~128 КБ. Полностью снят лимит в 100 МБ — теперь можно копировать файлы любого размера (3 ГБ+).
*   **📂 Рекурсивное копирование папок:** Полная поддержка копирования и вставки директорий со всей структурой вложенных файлов.
*   **🛡️ Предотвращение конфликтов (clip_<timestamp>):** Полученные файлы буфера сохраняются в уникальные папки в каталоге `Downloads/DeskBridge_Clipboard/` для предотвращения перезаписи.
*   **🍏 Устранение вылетов на macOS:** Чтение и запись системного буфера обмена происходят строго в главном UI-потоке ОС через `run_on_main_thread`, предотвращая краши Cocoa.
*   **🔗 Безопасная работа в мульти-ПК сетях:** Флаг `KVM_CLIENT_ACTIVE` исключает конфликт перехвата собственных симулированных событий ввода на управляемой машине.
...
*   **🔧 ...** (остальные изменения v2.6.1 сохранены)
